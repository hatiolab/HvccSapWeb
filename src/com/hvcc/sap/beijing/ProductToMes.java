...
public class ProductToMes {
	...
	public static final String INSERT_PRODUCT_SQL = "INSERT INTO INF_SAP_PRODUCT(IFSEQ, WERKS, MATNR, MAKTX, MTART, MEINS, MATKL, BESKZ, SOBSL, MMSTA, BSTRF, ERDAT, ERZET, ERNAM, AEDAT, AEZET, AENAM, IFRESULT, IFFMSG, MES_STAT, MES_ISTDT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSDATE)";
	...
	public void execute(String fromDateStr, String toDateStr) {
		...
		try {
			...
			if("S".equals(ifresult)) {
				...
				LOGGER.info("Got (" + resultCount + ") Product From SAP!");
				resultCount = this.createBomData(bomResults);
				LOGGER.info("Got (" + resultCount + ") BOM From SAP!");
			} else {
				LOGGER.info("Failed to get BOM From SAP!");
			}			
		} catch (Exception e) {
			LOGGER.severe("Failed to get BOM From SAP!" + e.getMessage());
		}	
	}
